...
class Products
{
	...
	public function addProduct($product_name,
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/TripWeb/product_images/".$uniqueImageName)) {
					...
					if ($q) {
						return ['status'=> 202, 'message'=> 'A product has been added!'];
					}else{
						return ['status'=> 303, 'message'=> 'DB error'];
					}

				}else{
					return ['status'=> 303, 'message'=> 'Problem with uploading the image'];
				}

			}else{
				return ['status'=> 303, 'message'=> 'Only 2MB Max. image size allowed'];
			}
	...
	public function editProductWithoutImage($pid,
		...
		if ($pid != null) {
			...
			return ['status'=> 303, 'message'=> 'Check product Id'];
		}
	...
	public function deleteProduct($pid = null){
		if ($pid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Product removed'];
			}else{
				return ['status'=> 202, 'message'=> 'DB error'];
			}
	...
	public function deleteCategory($cid = null){
		if ($cid != null) {
			...
			return ['status'=> 303, 'message'=>'Check category id'];
		}
	...
	public function deleteBrand($bid = null){
		if ($bid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Brand deleted'];
			}else{
				return ['status'=> 202, 'message'=> 'DB error'];
			}
