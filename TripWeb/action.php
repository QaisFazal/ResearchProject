...
if(isset($_POST["brand"])){
	...
	echo "
		<div class='nav nav-pills nav-stacked'>
			<li class='active'><a href='#'><h4>Duration</h4></a></li>
	";
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			$bid = $row["brand_id"];
				...
				<div class='col-md-4'>
							<div class='panel panel-info'>
								...
								<div class='panel-heading'>$.$pro_price.00
									...
									<a class='btn btn-success btn-xs' href='product_details.php?id=$pro_id'>Details</a>
								</div>
							</div>
						</div>	
			";
...
if (isset($_POST["Common"])) {
	...
	if (isset($_POST["checkOutDetails"])) {
		if (mysqli_num_rows($query) > 0) {
			//display user cart item with "You're not logged in!" button if user is not login
			echo "<form method='post' action='login_form.php'>";
				...
				if (!isset($_SESSION["uid"])) {
					echo '<input type="submit" style="float:right;" name="login_user_with_product" class="btn btn-info btn-lg" value="Login to proceed!" >
							</form>';
